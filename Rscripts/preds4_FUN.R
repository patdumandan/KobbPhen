plot_plant_preds4 <- function(species_name, data,
  ...
  for (yy in seq_along(year_ids)) {
    ...
    for (pl in seq_along(plot_ids)) {
      ...
      # Extract plot-by-year draw column
      u_plot_year_name <- paste0("u_plot_year[", pl_idx, ",", y_idx, "]")

      if (!u_plot_year_name %in% colnames(u_plot_year)) {
        stop("Could not find column in posterior draws: ", u_plot_year_name)
      }

      u_plot_year_draws <- u_plot_year[, u_plot_year_name]
      ...
      for (nd in seq_along(DOY)) {
        ...
        eta_post <-
          ...
          u_plot[, pl_idx] +
          u_plot_year_draws -
          ((DOY[nd] - mu_year[, y_idx])^2 / (width_year[, y_idx]^2))
